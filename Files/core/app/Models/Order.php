    ...
    protected static function booted()
{
    static::updated(function ($order) {
        // Garante que só vai rodar uma vez, ao virar pago
        if ($order->isDirty('payment_status') && $order->payment_status == 1 && empty($order->qr_code_data)) {
            // Gera o QR code
            $order->generateAndSaveQrCode(); // <-- sua função para gerar e salvar
        }
    });
}
