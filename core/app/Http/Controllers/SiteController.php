 ...
 public function downloadTicket($ticketId)
{
    ...
    // ✅ Gera o QR Code com base no campo qr_code_data
    $qrData = $ticket->qr_code_data ?? 'dados-default';
    $filename = 'qrcodes/qr_' . $ticket->id . '.png';
    $path = storage_path('app/public/' . $filename);

    // Gera o QR apenas se ainda não existir
    if (!Storage::disk('public')->exists($filename)) {
        Storage::disk('public')->put($filename, QrCode::format('png')->size(200)->generate($qrData));
    }

    // URL do QR Code para usar na view
$qrCodePath = storage_path('app/public/qrcodes/qr_' . $ticket->id . '.png');


    // ⚠️ Passa também o $qrCodeUrl para a view
$pdf = PDF::loadView('Template::user.event.ticket.invoice', compact('pageTitle', 'ticket', 'qrCodePath'));

    return $pdf->download('Ingresso - Trilheiros.pdf');
}
